...
const Employee = () => {
  ...
  const [formMode,setFormMode]=useState('N');
  ...
  const handleSavestudy = async () => {
    try {
      ...
      const response = await axios.post("http://localhost:5182/updateStudentdetails", postData);
      alert(response.data);
      ...
      <div className='form-container' style={{ position: "relative" }}>
        ...
        {emplist.length > 0 && (
          <div className='table-container'>
            ...
            <button type='button' onClick={handleSavestudy}>{formMode =='E' ? "Update" : "Save"}</button>
